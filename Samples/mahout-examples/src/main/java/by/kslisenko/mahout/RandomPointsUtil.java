...
public class RandomPointsUtil {
	...
	public static List<Vector> chooseRandomPoints(Iterable<Vector> vectors, int pointsNumber) {
		List<Vector> chosenPoints = new ArrayList<Vector>(pointsNumber);
		Random random = RandomUtils.getRandom();
		for (Vector value : vectors) {
			int currentSize = chosenPoints.size();
			if (currentSize < pointsNumber) {
				chosenPoints.add(value);
				// with chance 1/(currentSize+1) pick new element evict one chosen randomly
